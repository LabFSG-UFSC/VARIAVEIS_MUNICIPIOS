#!/usr/bin/env python3
"""
Clusteriza a classe A priorizando:
- domicilios CNEFE
- populacao total
...
SAIDA_BASE_PADRAO = ROOT / "classificacao_municipios" / "processamento" / "clusters_classe_a_foco_cnefe_pop_setores_ofertas_municipios.csv"
SAIDA_RESUMO_PADRAO = ROOT / "classificacao_municipios" / "processamento" / "clusters_classe_a_foco_cnefe_pop_setores_ofertas_resumo.csv"
SAIDA_CENTROIDES_PADRAO = ROOT / "classificacao_municipios" / "processamento" / "clusters_classe_a_foco_cnefe_pop_setores_ofertas_centroides.csv"
SAIDA_GPKG_PADRAO = ROOT / "classificacao_municipios" / "processamento" / "clusters_classe_a_foco_cnefe_pop_setores_ofertas_municipios.gpkg"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clusteriza a classe A com foco em CNEFE, populacao, setores e ofertas."
    )
...
def monta_centroides(base_valida: pd.DataFrame, cluster_rank: pd.Series, medias: pd.Series, desvios: pd.Series) -> pd.DataFrame:
    trabalho = base_valida.copy()
    trabalho["cluster_foco"] = cluster_rank.values
    agrupado = trabalho.groupby("cluster_foco")[VARIAVEIS_CLUSTER].mean().reset_index()
...
def monta_resumo(df: pd.DataFrame, inertia: float) -> pd.DataFrame:
    resumo = (
        df.groupby("grupo_foco", as_index=False)
        .agg(
        ...
        .sort_values("domicilios_totais", ascending=False)
        .reset_index(drop=True)
    )
...
def main() -> None:
    ...
    X, medias, desvios, idx_validos = padroniza(base, VARIAVEIS_CLUSTER)
    ...
    base_valida["cluster_foco"] = cluster_rank.values
    base_valida["grupo_foco"] = base_valida["cluster_foco"].map(lambda x: f"foco_a_{int(x)}")

    resumo = monta_resumo(base_valida, inertia)
    centroides = monta_centroides(base_valida, cluster_rank, medias, desvios)
    ...
    base_valida.sort_values(["grupo_foco", "domicilios"], ascending=[True, False]).to_csv(output_base, index=False)
    resumo.to_csv(output_resumo, index=False)
    centroides.to_csv(output_centroides, index=False)
    gdf.to_file(output_gpkg, layer="clusters_classe_a_foco", driver="GPKG")
